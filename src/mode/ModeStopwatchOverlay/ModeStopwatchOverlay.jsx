...
function ModeStopwatchOverlay() {
  ...
  const verifyInput = useCallback(() => {
    ...
    timeAdd1 =
      (timeAdd1[0] == "" ? 0 : parseInt(timeAdd1[0])) * 60 +
      parseInt(timeAdd1[1]);
    timeAdd2 =
      (timeAdd2[0] == "" ? 0 : parseInt(timeAdd2[0])) * 60 +
      parseInt(timeAdd2[1]);

    if (
      timeAdd1 === time + (penalties * 30 - 10) &&
  ...
  useEffect(() => {
    const onResize = () => {
      ...
      module.current.style.setProperty(
        "--stopwatch-container-height",
        `${stopwatchContainerHeight}px`
      );
    };
    onResize();
    ...
    >
      ...
      {timeAttackStarted === TIME_ATTACK_STATES.RESULTS ? (
        <div className={styles.resultsOverlay}>
          ...
          <div>
            <p>
              You solved: {timeAttackSolvedCount} times (
              {timeAttackSolvedCount * 2} additions)
            </p>
            <button type="button" onClick={timeAttackStop}>
              Great!
            </button>
          </div>
        </div>
      ...
      <ValidationResponse isValid={isValid} validationCount={validationCount} />
    </form>
  );
}
