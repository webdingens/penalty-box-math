...
class Input extends React.Component {
  ...
  componentDidUpdate(prevProps) {
    if (prevProps.settings.splitInput && !this.props.settings.splitInput && this.state.value2 !== '') {
      this.setState({
        value1: this.state.value1 + ':' + this.state.value2,
        value2: ''
      })
    }
    if (!prevProps.settings.splitInput && this.props.settings.splitInput && this.input1Ref.current.value !== '') {
